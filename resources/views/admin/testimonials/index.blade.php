                            ...
                            <div class="listjs-table" id="testimonialList">
                                <div class="row g-4 mb-3">
                                    <div class="col-sm-auto">
                                        <div>
                                            <a href="{{ route('admin.testimonials.create') }}" class="btn btn-primary add-btn">
                                            ...
                                            disabled
                                            data-action="{{ route('admin.testimonials.bulk-destroy') }}"
                                        ...
                                        <tbody class="list form-check-all">
                                            @foreach ($testimonials as $testimonial)
                                                <tr>
                                                    <th>
                                                        <div class="form-check">
                                                            <input class="form-check-input chk-child" type="checkbox" name="chk_child" value="{{ $testimonial->id }}">
                                                        </div>
                                                    </th>
                                                    <td class="name">{{ $testimonial->name }}</td>
                                                    <td class="designation">{{ $testimonial->designation }}</td>
                                                    <td class="image">
                                                        @if($testimonial->image)
                                                            <img src="{{ asset('storage/' . $testimonial->image) }}" alt="{{ $testimonial->name }}" style="max-width:50px; max-height:50px;">
                                                        @else
                                                    ...
                                                    <td class="status">
                                                        <form action="{{ route('admin.testimonials.toggle-status', $testimonial->id) }}" method="POST">
                                                            ...
                                                            <div class="form-check form-switch">
                                                                <input class="form-check-input status-toggle" type="checkbox" role="switch" id="status-{{ $testimonial->id }}" {{ $testimonial->status ? 'checked' : '' }} onchange="this.form.submit()">
                                                                <label class="form-check-label" style="padding-left:40px;" for="status-{{ $testimonial->id }}">{{ $testimonial->status ? 'Active' : 'Inactive' }}</label>
                                                            </div>
                                                        </form>
                                                    </td>
                                                    <td>
                                                        <div class="d-flex gap-2">
                                                            ...
                                                            <form action="{{ route('admin.testimonials.destroy', $testimonial->id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="button" class="btn btn-sm btn-danger js-single-delete" data-message="Are you sure you want to delete this testimonial?">Remove</button>
                                                            </form>
                                                        </div>
                                                    </td>
                                                </tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Initialize TableManager specifically for Testimonials
    ...
    showCountOnButton: false   // <-- set this to false to remove count/text
});
