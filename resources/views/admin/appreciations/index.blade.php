            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">Appreciations</h4>
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="{{ route('admin.appreciations.index') }}">Appreciations</a></li>
                                <li class="breadcrumb-item active">Appreciation List</li>
                            </ol>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title mb-0">Manage Appreciations</h4>
                        </div><!-- end card header -->
                            ...
                            <div class="listjs-table" id="customerList">
                                <div class="row g-4 mb-3">
                                    <div class="col-sm-auto">
                                        <div>
                                            <a href="{{ route('admin.appreciations.create') }}" class="btn btn-primary add-btn"><i class="ri-add-line align-bottom me-1"></i> Add</a>
                                ...
                                <div class="table-responsive table-card mt-3 mb-1">
                                    <table class="table align-middle table-nowrap" id="customerTable">
                                        ...
                                        <tbody class="list form-check-all">
                                            @foreach ($appreciations as $testimonial)
                                                <tr>
                                                    <th scope="row">
                                                        <div class="form-check">
                                                    ...
                                                    <td class="image">
                                                        @if ($testimonial->image)
                                                            <img src="{{ asset('storage/appreciations/' . $testimonial->image) }}" alt="{{ $testimonial->name ?? 'Appreciation Image' }}" style="max-width: 50px; max-height: 50px; object-fit: cover;">
                                                        @else
                                                            <span>No Image</span>
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td class="status">
                                                        <form action="{{ route('admin.appreciations.toggle_status', $testimonial->id) }}" method="POST" class="status-form">
                                                            @csrf
                                                            @method('PATCH')
                                                    ...
                                                    <td>
                                                        <div class="d-flex gap-2">
                                                            <a href="{{ route('admin.appreciations.edit', $testimonial->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                                            <form action="{{ route('admin.appreciations.destroy', $testimonial->id) }}" method="POST" class="delete-form" style="display:inline;">
                                                                @csrf
                                                                @method('DELETE')
                                    ...
                                    <div class="noresult" style="display: none">
                                        <div class="text-center">
                                            ...
                                            <p class="text-muted mb-0">No appreciations found for your search.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-flex justify-content-end">
                                    <div class="pagination-wrap hstack gap-2">
                                        <a class="page-item pagination-prev {{ $appreciations->previousPageUrl() ? '' : 'disabled' }}" href="{{ $appreciations->previousPageUrl() }}">Previous</a>
                                        <ul class="pagination listjs-pagination mb-0">
                                            @for ($i = 1; $i <= $appreciations->lastPage(); $i++)
                                                <li class="page-item {{ $appreciations->currentPage() == $i ? 'active' : '' }}">
                                                    <a class="page-link" href="{{ $appreciations->url($i) }}">{{ $i }}</a>
                                                </li>
                                            @endfor
                                        </ul>
                                        <a class="page-item pagination-next {{ $appreciations->nextPageUrl() ? '' : 'disabled' }}" href="{{ $appreciations->nextPageUrl() }}">Next</a>
                                    </div>
                                </div>
                            </div>
                        </div><!-- end card -->
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelector('#delete-multiple-btn').addEventListener('click', function () {
            ...
            if (confirm(`Are you sure you want to delete ${checkedIds.length} testimonial(s)?`)) {
                ...
                form.method = 'POST';
                form.action = '{{ route("admin.appreciations.bulk_destroy") }}';
                form.innerHTML = `
                    @csrf
